                                ...
                                <div class="run-section">
                                    ...
                                    <div class="execution-settings">
                                        ...
                                        <div class="setting-group">
                                            <label>任务间隔(秒):</label>
                                            <input type="number" value="30" min="1" id="intervalMin"> - 
                                            <input type="number" value="60" min="1" id="intervalMax">
                                            <input type="checkbox" id="randomInterval" checked>
                                            <label for="randomInterval">随机</label>
                                        </div>
                                        <div class="setting-group">
                                            <label>Gas 策略:</label>
...
function collectConfigFormData(container) {
    ...
    // 收集运行配置（如果存在）
    try {
        const intervalMin = container.querySelector('#intervalMin');
        if (intervalMin) {
            config.intervalMin = parseFloat(intervalMin.value) || 1;
        }
        
        const intervalMax = container.querySelector('#intervalMax');
        if (intervalMax) {
            config.intervalMax = parseFloat(intervalMax.value) || 1;
        }
        
        const randomInterval = container.querySelector('#randomInterval');
        if (randomInterval) {
            config.randomInterval = randomInterval.checked;
        }
        
        const gasStrategy = container.querySelector('#gasStrategy');
        if (gasStrategy) {
            config.gasStrategy = gasStrategy.value;
        }
    } catch (e) {
        console.error('收集运行配置出错:', e);
    }
