...
*   **钱包管理 (功能已完善)**：
    ...
    *   **按需解密 (新!)**: 点击"查看详情"时，应用会安全地解密并显示私钥/助记词。
    ...
    *   **界面交互**: 
        ...
        *   **加密数据显示 (新!)**: 表格中加密的私钥和助记词会以缩短形式（如 `AGy8OI***GNCc`）显示，避免直接暴露完整密文。
...
*   **安全机制 (新!)**:
    ...
    *   **密码验证**: 通过存储加密验证字符串来验证用户输入的密码，确保配置未被篡改。
    *   **配置存储**: 加密所需的盐值、迭代次数和验证密文存储在用户数据目录下的 `encryption.config.json`。
    *   **加密核心**: 使用 PBKDF2 从主密码派生加密密钥，并使用 AES-GCM 对敏感数据进行加密。
    *   **应用解锁**: 若未自动解锁，启动时要求输入主密码以加载会话密钥。
    *   **自动解锁 (新!)**: 首次成功解锁后，派生的主密钥会安全地存储在操作系统的凭据管理器中（使用 `keytar`，如 macOS Keychain 或 Windows Credential Manager）。后续启动时，应用会尝试自动从系统加载密钥，通常无需再次输入密码。
    *   **手动锁定 (新!)**: 提供 IPC 接口 (`app:lock`) 清除内存中的密钥并从系统凭据中删除已存密钥，强制下次启动或调用时需要密码。(UI 按钮待添加)
...
*   **安全提示**: 
    *   应用已实现基于主密码的 AES-GCM 加密来保护存储的敏感数据。
    *   为了方便使用，解锁后的主密钥会尝试存储在您操作系统的安全凭据管理器中（依赖 `keytar`）。这通常意味着您的系统登录密码或生物识别信息也保护着这个密钥。
    *   请务必 **牢记并妥善保管您首次设置的主密码**，它是恢复访问的根本。同时，确保您的操作系统账户安全。
    *   丢失主密码且无法访问系统凭据将导致加密数据 **永久丢失**！
    *   请勿在不安全的计算机上使用本应用。
