...
*   **钱包管理 (功能已完善)**:
    ...
    *   **钱包-社交账户关联 (新!)**: 
        *   支持将钱包与已添加的社交账户（Twitter, Discord, Email 等）进行关联。
        *   钱包列表新增"关联账户"列，用平台图标和数量徽章简洁展示关联状态。
        *   点击该列可打开管理模态框，通过标签页（所有、已关联、各平台）、搜索和分页（针对"所有"和"已关联"）高效管理关联关系。
    *   **界面交互**:
...
*   **数据库模块结构重构**: 按功能拆分到 `src/js/db/` 下的 `group.js`, `wallet.js`, `social.js`。
...
*   **钱包-社交账户关联 (新!)**: 新增钱包与社交账户关联功能，优化了管理模态框（标签页、搜索、分页）和表格显示（图标+徽章）。