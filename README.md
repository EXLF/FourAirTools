...
*   [x] **实现数据显示/使用时的解密逻辑。**
*   [x] **社区交流功能**: 实现Discord社区嵌入，支持自动保存会话状态，提供退出和切换账户功能。
...
*   **社区交流Discord优化 (功能已完善)**:
        *   优化Discord页面体验，首次点击"加入Discord"后会保存Discord频道URL
        *   后续访问社区页面时自动打开上次访问的Discord频道，避免重复显示邀请页面
        *   自动记录用户在Discord中导航的位置，确保下次打开时显示上次浏览的位置
        *   添加退出账户按钮（位于页面标题栏右侧），支持用户切换Discord账户或退出当前会话
        *   使用自定义确认对话框替代原生弹窗，提供统一的用户界面体验
        *   优化确认对话框显示位置和样式，确保在页面中央正确展示
        *   提供背景点击关闭功能，增强用户交互体验
        *   实现完整的会话管理，退出后清除会话数据，允许用户重新加入
        *   实现更流畅的用户体验，无需重复接受邀请
*   **IP代理管理模块完善**:
    *   完成代理配置的增删改查、连通性测试、出口IP检测等功能
    *   支持 HTTP/HTTPS/SOCKS5 代理类型
    *   实现批量测试、批量删除功能
    *   添加导入/导出功能，支持多种格式
    *   优化界面交互和数据展示
    *   实现全局代理应用功能
...
*   **项目跟踪功能增强 (Web3 定制)**:
    *   采用**左右布局**，左侧为项目列表，右侧为详情面板，更符合Web3撸毛场景。
    *   项目列表支持**按链筛选**（Solana, Optimism, Base, Arbitrum等）和状态标记（待打卡/已打卡/更新/剩余天数）。
    *   右侧详情面板包含**任务列表**、**打卡日历**和**项目时间轴**。
    *   **项目时间轴**模块：清晰展示项目从启动到结束的关键节点和活动，支持倒序显示（最新在前）并添加独立滚动条。
    *   **移除Electron默认菜单栏**，提供更简洁的应用界面。
