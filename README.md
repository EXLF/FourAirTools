...
*   **打包与路径修复 (2024-05-22)**
    *   修复了 Electron 应用打包后数据库路径错误的问题，确保在打包应用中能正确访问位于用户数据目录 (`userData`) 下的数据库文件 (`src/js/db/index.js`)。
    *   修复了在IP代理管理模块中，添加新代理后列表不会自动刷新的问题，通过事件机制实现了实时更新 (`src/js/pages/network/modules/modals.js`, `src/js/pages/network/modules/events.js`)。
    *   **脚本执行引擎代理功能增强 (进行中)**:
        *   已定位问题：脚本执行引擎 (`src/main/scriptEngine.js`) 当前仅接收代理ID，未从数据库查询完整的代理详细信息（如主机、端口、认证信息）并配置给脚本沙箱环境中的网络请求模块（如 `axios`）。
        *   计划改进：`scriptEngine.js` 将在接收到代理ID后，从数据库获取完整的代理配置，并正确设置脚本沙箱环境，确保脚本能通过指定代理进行网络请求。
